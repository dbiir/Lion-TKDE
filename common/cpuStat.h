...
namespace get_system_usage_linux
{
    ...
    struct CPU_stats
    {
        ...
        long long get_total_idle()
        const {
            return idle + iowait;
        }

        long long get_total_active()
        const {
    ...
    inline CPU_stats read_cpu_data()
    {
        ...
        if (proc_stat.good())
        {
            ...
            unsigned long long *stats_p = (unsigned long long *)&result;
    ...
    inline double get_cpu_usage(const CPU_stats &first, const CPU_stats &second) {
        const double active_time	= static_cast<double>(second.get_total_active() - first.get_total_active());
		const double idle_time	= static_cast<double>(second.get_total_idle() - first.get_total_idle());
